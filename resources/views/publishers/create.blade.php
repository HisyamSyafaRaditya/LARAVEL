...
<div class="px-4 sm:px-0 max-w-3xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow p-8">
        <h1 class="text-2xl font-bold text-gray-900 mb-2">Add Publisher</h1>
        <p class="text-gray-600 mb-6">Create a new publisher entry</p>
        ...
        <form action="{{ route('publishers.store') }}" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <textarea id="p_address" name="p_address" rows="3"
                          ...
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('p_address') }}</textarea>
                @error('p_address')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
